...
// 🌄 New: Background video elements (2 for each mode)
const darkVideos = [
    document.getElementById("darkVideo1"),
    document.getElementById("darkVideo2")
];
const lightVideos = [
    document.getElementById("lightVideo1"),
    document.getElementById("lightVideo2")
];
...
function playLoopingVideo(mode) {
    const videos = mode === 'dark' ? darkVideos : lightVideos;
    let index = mode === 'dark' ? currentDarkIndex : currentLightIndex;
    const nextIndex = (index + 1) % 2;

    // Hide all videos of both modes
    [...darkVideos, ...lightVideos].forEach(v => {
        v.pause();
    ...
    // Set next index for the next loop
    if (mode === 'dark') currentDarkIndex = nextIndex;
    else currentLightIndex = nextIndex;

    // Play next video when this one ends
    currentVideo.onended = () => playLoopingVideo(mode);
}
...
// Start initial dark mode background video loop
...
// ✅ Show redirect modal and go to external certificate URL
function showModalAndRedirect(buttonElement) {
